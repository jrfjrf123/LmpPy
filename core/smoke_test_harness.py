...
- MPI 感知：rank 0 负责准备和验证，所有 rank 参与模拟
...
class SmokeTestHarness:
    ...
    使用示例:
        ...
        report = harness.run()
        if report is not None:  # None on non-rank-0
            report.print()
    ...
    def run(self) -> Optional[SmokeTestReport]:
        """
        执行冒烟测试的完整流程（MPI 感知）。

        Rank 0: 创建临时目录 → 复制配置 → 运行模拟 → 验证 → 清理
        其他 rank: 仅参与模拟，返回 None

        Returns:
            SmokeTestReport (rank 0) 或 None (其他 rank)
        """
        # ---- Rank 0: 准备工作 ----
        if _me == 0:
        ...
        # 广播 temp_dir 路径给所有 rank
        if _comm is not None:
            temp_dir_str = _comm.bcast(str(self.temp_dir) if _me == 0 else None, root=0)
            if _me != 0:
